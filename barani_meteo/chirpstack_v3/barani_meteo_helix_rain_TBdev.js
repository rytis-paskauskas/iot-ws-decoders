// Monolithic Payload Decoder for Barani MeteoHelix IoT, MeteoRain___, MeteoWind IoT 
// Special case of cable-connected meteoRain to meteoHelix
// Time-stamp: <2025-04-02 16:31:54 rytis>
...
function Decode(fPort, bytes)
{
	...
	while (bitsRead < n) {
            var byteIndex = Math.floor(this.bitOffset / 8);
            var bitIndex = this.bitOffset % 8;
	    
            if (byteIndex >= this.bytes.length) {
		throw "Not enough bits in buffer";
            }

            var currentByte = this.bytes[byteIndex];
            var bitsLeftInByte = 8 - bitIndex;
            var bitsToRead = Math.min(n - bitsRead, bitsLeftInByte);

            var shift = bitsLeftInByte - bitsToRead;
            var mask = (1 << bitsToRead) - 1;
            var extracted = (currentByte >> shift) & mask;
    ...
    bitReader.prototype.hasBits = function(n) {
	return (this.bitOffset + n) <= (this.bytes.length * 8);
    };
    ...
    // ts: Transmitter unix timestamp with milliseconds
	...
	if (dateString.length>=19) {
	    msg.ts = localTsMs(dateString);
	}
    }


    // meteoHelix cable connected with meteoRain aka HelixRain
    // https://www.baranidesign.com/meteohelix-message-decoder
    if (bytes.length === 11 && fPort === 1) {
	asset_type = "meteoHelixRain";
	Type =  reader.readBits(2);
	Battery = Round(reader.readBits(5)*0.05+3, 2);
	Temperature = Round(reader.readBits(11)*0.1-100, 1);
	T_min = Round(Temperature - reader.readBits(6)*0.1, 1);
	T_max = Round(Temperature + reader.readBits(6)*0.1, 1);
	Humidity = Round(reader.readBits(9)*0.2, 1);
	Pressure = reader.readBits(14)*5+50000;
	Irradiation = reader.readBits(10)*2;
	Irr_max = Irradiation + reader.readBits(9)*2;
	Rain = Round(reader.readBits(8), 1);
	Rain_time = Round(reader.readBits(8), 1);
	...
	Index = Round(reader.readBits(8)*1, 1);
	Battery = Round(reader.readBits(3)*0.2+3, 1);
	Wind_ave = Round(reader.readBits(9)*0.1, 1);
	Wind_3sgust = Round(Wind_ave + reader.readBits(9)*0.1, 1);
	Wind_3smin = Round(Wind_ave - reader.readBits(9)*0.1, 1);
	Wind_stdev = Round(reader.readBits(8)*0.1, 1);
	Dir_ave = Round(reader.readBits(9)*1, 1);
	Dir_3sgust = Round(reader.readBits(9)*1, 1);
	Dir_stdev = Round(reader.readBits(7)*1, 1);
	Gust_time = Round(reader.readBits(7)*5, 1);
	Vector_scalar = Round(reader.readBits(1)*1, 1);
	Alarm_sent = Round(reader.readBits(1)*1, 1); 
	...
	Type =  reader.readBits(2);
	Battery = Round(reader.readBits(5)*0.05+3, 1);
	Wind_ave10 = Round(reader.readBits(9)*0.1, 1);
	Wind_max10 = Round(Wind_ave10 + reader.readBits(9)*0.1, 1);
	Wind_min10 = Round(Wind_ave10 - reader.readBits(9)*0.1, 1);
	Dir_ave10 = Round(reader.readBits(9)*1, 1);
	Dir_max10 = Round(reader.readBits(9)*1, 1);
	Dir_hi10 = Round(reader.readBits(8)*1, 1);
	Dir_lo10 = Round(reader.readBits(8)*1, 1);
	...
	asset_type = "unidentifiedBarani";
	decoded = {
	    "length": bytes.length,
	    "port": fPort,
	    "payloadHex": hex,
	    "info": "Suspected duplication of rain parameters. Use barani_meteo_iot_TBdev.js for meteoRain IoT, barani_meteo_helix_rain_TBdev.js for Helix-cable-Rain. Cannot have both."
	};
    } else {
	//===================================================================
